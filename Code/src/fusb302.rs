...
impl<'a> Fusb<'a> {
    pub fn new(i2c: I2cDriver<'a>, i2c_address: u8) -> Fusb<'a> {
...
impl Fusb<'_> {
    ...
    fn write_reg(&mut self, reg: u8, val: u8) -> Result<(), EspError> {
        self.i2c.write(self.i2c_address, &[reg, val], 0)
    }
    fn read_reg_to_buf(&mut self, reg: u8, buf: &mut u8) -> Result<(), EspError> {
        self.i2c
            .write_read(self.i2c_address, &[reg], &mut [*buf], 0)
    }

    #[inline]
    fn read_reg(&mut self, reg: u8) -> Result<u8, EspError> {
        let mut buf: [u8; 1] = [0];
        self.i2c.write_read(self.i2c_address, &[reg], &mut buf, 0)?;
        Ok(buf[0])
    }
    fn write_bmc(&mut self, data: &[u8]) -> Result<(), EspError> {
        self.i2c
            .write(self.i2c_address, &[&[0x43], data].concat(), 0)
    }
    fn read_bmc(&mut self, data: &mut [u8]) -> Result<(), EspError> {
        self.i2c.write_read(self.i2c_address, &[0x43], data, 0)
    }
}
